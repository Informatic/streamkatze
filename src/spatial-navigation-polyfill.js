...
(function () {
    ...
    if (
      ...
      if (
        ...
        if (getCSSSpatNavAction(eventTarget) === 'scroll') {
          ...
          bestInsideCandidate = eventTarget.spatialNavigationSearch(
            dir,
            {
              container: eventTarget,
              candidates: getSpatialNavigationCandidates(
                eventTarget,
                { mode: 'all' }
              ),
            }
  ...
  function focusingController(bestCandidate, dir) {
    ...
    if (bestCandidate) {
      ...
      if (
        container !== window &&
        getCSSSpatNavAction(container) === 'focus'
      ) {
        bestCandidate.focus();
      } else {
  ...
  function scrollingController(container, dir) {
    ...
    if (
      isScrollable(container, dir) &&
      !isScrollBoundary(container, dir)
    ) {
    ...
    if (
      ...
      !isHTMLScrollBoundary(container, dir)
    ) {
  ...
  function getSpatialNavigationCandidates(
    ...
    if (container.childElementCount > 0) {
      if (!container.parentElement) {
        container =
          container.getElementsByTagName('body')[0] || document.body;
      }
  ...
  function getFilteredSpatialNavigationCandidates(
    ...
    container =
      container || targetElement.getSpatialNavigationContainer();
  ...
  function spatialNavigationSearch(dir, args) {
    ...
    let insideOverlappedCandidates = getOverlappedCandidates(
      targetElement
    );
    let bestTarget;
    ...
    let defaultCandidates = getSpatialNavigationCandidates(
      defaultContainer
    );
    ...
    if (candidates && candidates.length > 0) {
      ...
      let overlappedContainer = candidates.filter(
        (candidate) =>
          ...
          isEntirelyVisible(targetElement, candidate)
      );
      ...
      if (bestTarget && isDelegableContainer(bestTarget)) {
        ...
        const innerTarget = getSpatialNavigationCandidates(
          bestTarget,
          { mode: 'all' }
        );
  ...
  function getClosestElement(
    ...
    if (
      window.location !== window.parent.location &&
      (currentElm.nodeName === 'BODY' ||
        currentElm.nodeName === 'HTML')
    ) {
      ...
      eventTargetRect =
        searchOriginRect || currentElm.getBoundingClientRect();
    }
  ...
  function isCSSSpatNavContain(element) {
    return (
      readCssVar(element, 'spatial-navigation-contain') === 'contain'
    );
  }
  ...
  function navigateChain(
    ...
    if (
      getCSSSpatNavAction(container) === 'auto' &&
      option === 'visible'
    ) {
      if (scrollingController(container, dir)) return;
    }
  ...
  function findSearchOrigin() {
    ...
    if (
      ...
      if (
        ...
        if (
          ...
          invisibleStyle.includes(
            elementStyle.getPropertyValue('visibility')
          )
        ) {
          searchOrigin = savedSearchOrigin.element;
          return searchOrigin;
    ...
    if (
      savedSearchOrigin.element &&
      (getBoundingClientRect(savedSearchOrigin.element).height ===
        0 ||
    ...
    if (!isVisibleInScroller(searchOrigin)) {
      ...
      if (
        scroller &&
        (scroller === window ||
          getCSSSpatNavAction(scroller) === 'auto')
      )
        return scroller;
    }
    return searchOrigin;
  ...
  function isDelegableContainer(element) {
    return (
      readCssVar(element, 'spatial-navigation-contain') ===
      'delegable'
    );
  }
    ...
    if (element && typeof element === 'object') {
      if (dir && typeof dir === 'string') {
        ...
        if (isOverflow(element, dir)) {
          // style property
          const elementStyle = window.getComputedStyle(element, null);
          const overflowX = elementStyle.getPropertyValue(
            'overflow-x'
          );
          const overflowY = elementStyle.getPropertyValue(
            'overflow-y'
          );
  ...
  function isExpresslyInert(element) {
    return (
      element.inert && !element.ownerDocument.documentElement.inert
    );
  }
  ...
  function isVisibleStyleProperty(element) {
    const elementStyle = window.getComputedStyle(element, null);
    const thisVisibility = elementStyle.getPropertyValue(
      'visibility'
    );
    ...
    return (
      thisDisplay !== 'none' &&
      !invisibleStyle.includes(thisVisibility)
    );
  }
  ...
  function hitTest(element) {
    ...
    const hitTestPoint = {
      ...
      leftTop: [
        elementRect.left + offsetX,
        elementRect.top + offsetY,
      ],
      rightBottom: [
        elementRect.right - offsetX,
        elementRect.bottom - offsetY,
      ],
    };
    ...
    for (const point in hitTestPoint) {
      ...
      if (
        element === elemFromPoint ||
        element.contains(elemFromPoint)
      ) {
        return true;
      }
    }
  ...
  function getDistance(searchOrigin, candidateRect, dir) {
    ...
    // Get exit point, entry point -> {x: '', y: ''};
    const points = getEntryAndExitPoints(
      dir,
      searchOrigin,
      candidateRect
    );
    ...
    switch (dir) {
      ...
      case 'down':
        ...
        if (isAligned(searchOrigin, candidateRect, dir))
          alignBias = Math.min(
            intersectionRect.width / searchOrigin.width,
            1
          );
  ...
  function getIntersectionRect(rect1, rect2) {
    ...
    intersection_rect.width = Math.abs(
      new_location[0] - new_max_point[0]
    );
    intersection_rect.height = Math.abs(
  ...
  function handlingEditableElement(e) {
    ...
    if (
      SPINNABLE_INPUT_TYPES.includes(
        eventTarget.getAttribute('type')
      ) &&
      (dir === 'up' || dir === 'down')
    ) {
  ...
  function getExperimentalAPI() {
    ...
    function findTarget(findCandidate, element, dir, option) {
      ...
      // find the best candidate within startingPoint
      if (
        (isContainer(eventTarget) ||
          eventTarget.nodeName === 'BODY') &&
        !(eventTarget.nodeName === 'INPUT')
      ) {
        if (eventTarget.nodeName === 'IFRAME')
          eventTarget = eventTarget.contentDocument.body;

        const candidates = getSpatialNavigationCandidates(
          eventTarget,
          option
        );
        ...
        if (Array.isArray(candidates) && candidates.length > 0) {
          return findCandidate
            ...
            : eventTarget.spatialNavigationSearch(dir, {
                candidates,
              });
        }
    ...
    return {
      ...
      getDistanceFromTarget: (element, candidateElement, dir) => {
        if (
          ...
          if (
            getSpatialNavigationCandidates(element).includes(
              candidateElement
            )
          ) {
